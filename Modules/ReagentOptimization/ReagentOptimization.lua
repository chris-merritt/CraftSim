--[[
    ...
    Maximum skill bonus for a87 recipe from reagent quality
    ...
    for h = 0, #BPs, 1 do -- #BPs gives 1 less than how many are in there so here it fits!
        ...
        if BPs[h] >= 0 then --can reach this BP
            ...
            for i = numReagents, 0, -1 do
                ...
                local crumb = ks[i].compositions[k] -- to work around the single crumb of patron order reagents
                if crumb then
...
---@param ksItem CraftSim.REAGENT_OPTIMIZATION.REAGENT
---@param useSubRecipeCosts boolean
function CraftSim.REAGENT_OPTIMIZATION:CreateSimplifiedCrumbs(ksItem, useSubRecipeCosts)
    local requiredQuantity = ksItem.numReq
...
function CraftSim.REAGENT_OPTIMIZATION:OptimizeReagentAllocation(recipeData, maxQuality)
    ...
    -- The greatest common divisor of the weights is derived here to simplify the factors used in later calculation and thereby reduce overall permutation count (i.e. crumbs)
    ...
    for index = 0, #requiredReagents - 1, 1 do
        ...
        if recipeData:IsSimplifiedQualityRecipe() then
            CraftSim.REAGENT_OPTIMIZATION:CreateSimplifiedCrumbs(ksItem, recipeData.subRecipeCostsEnabled)
        else
